...
def popular_articles():
    ...
    curz = db.cursor()
    curz.execute("select articles.title, count(*) as views from articles join log on concat('/article/', articles.slug) = log.path group by articles.title order by views DESC")  # noqa
...
def popular_authors():
    ...
    db = psycopg2.connect(database=DBNAME)
    curz = db.cursor()
    curz.execute("select authors.name, count(*) as views from articles, authors, log where concat('/article/',articles.slug) = log.path and authors.id = articles.author group by authors.name order by views DESC")  # noqa
    result = curz.fetchall()
    db.close()
    with open("news_output.txt", "a") as text_file:
        text_file.write("============================================================" + '\r\n')  # noqa
